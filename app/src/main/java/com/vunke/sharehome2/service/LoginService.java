package com.vunke.sharehome2.service;
...
import com.vunke.sharehome2.url.UrlClient;
import com.vunke.sharehome2.utils.Encrypt3DES;
...
public class LoginService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		WorkLog.e("LoginService", "LoginService---onStartCommand");
		if (intent != null) {
	...
	private void initService(Intent intent, String action) {
		if (action.equals(login_sdk.LOGIN_ACTION)) {
			...
			if (channel_Id == -1) {
				...
				login_sdk.setLOGIN_STATUS(LoginSDK.LOGIN_ERROR);
				SendMessage();
				WorkLog.e("LoginService", "获取渠道ID失败");
				return;
			}
			...
			if (timestamp == -1) {
				...
				login_sdk.setLOGIN_STATUS(LoginSDK.LOGIN_ERROR);
				SendMessage();
				WorkLog.e("LoginService", "获取登录时间失败");
				return;
			}
	...
	private void initLogin() {
		try {
			String str = LoginSDK.key + login_sdk.getChannel_Id()
					+ login_sdk.getMobile_no() + login_sdk.getTimestamp();
			WorkLog.e("LoginService", "正在验证签名信息");
			...
			if (md5.equals(login_sdk.getSign())) {
				...
				SendMessage();
				StartGetPassword();
				WorkLog.e("LoginService", "验证签名信息成功，开始登录");
			} else {
				login_sdk.setLOGIN_RESON(LoginSDK.RESON_SIGN_IS_ERROR);
				login_sdk.setLOGIN_STATUS(LoginSDK.LOGIN_ERROR);
				SendMessage();
				WorkLog.e("LoginService", "验证签名信息失败");
			}
		} catch (Exception e) {
			// e.printStackTrace();
			login_sdk.setLOGIN_RESON(LoginSDK.RESON_SIGN_IS_ERROR);
			login_sdk.setLOGIN_STATUS(LoginSDK.LOGIN_ERROR);
			SendMessage();
			WorkLog.e("LoginService", "验证签名信息出现异常");
		}
	...
	private void StartGetPassword() {
		if (login_sdk.isLogin() == true) {
			...
			login_sdk.setLOGIN_STATUS(LoginSDK.LOGIN_ERROR);
			SendMessage();
			WorkLog.e("LoginService", "正在登录中，请不要重复请求");
			return;
		}
	...
	private void getPassword() {
		...
		try {
			...
			json.put("userType", "9");
			WorkLog.e("LoginService", "request json:" + json.toString());
			OkHttpUtils.post(UrlClient.HttpUrl + UrlClient.GetkeyURL).tag(this)
					.params("json", json.toString())
					.execute(new StringCallback() {

						@Override
						public void onResponse(boolean isFromCache,
								@Nullable String t, Request request,
								@Nullable Response response) {
							WorkLog.e("LoginService", "get request" + t);
							...
							try {
								...
								switch (code) {
								case 200:
									WorkLog.e("LoginService",
											"reueset success:");
									String encodePass = js
											.getString("encodePass");
									// String firstLogin =
									// js.getString("firstLogin");
								...
								case 400:
								case 500:
									WorkLog.e("LoginService", "code:" + code);
						...
						public void onError(boolean isFromCache, Call call,
							...
							WorkLog.e("LoginService", "OnError");
							login_sdk.setGetpassword(false);
							login_sdk
									.setLOGIN_RESON(LoginSDK.RESON_MOBILE_AUTH_ERROR);
							login_sdk.setLOGIN_STATUS(LoginSDK.LOGIN_ERROR);
							SendMessage();
						}
						...
						public void onAfter(boolean isFromCache,
							...
							if (TextUtils.isEmpty(login_sdk.getPassWord())) {
								login_sdk.setGetpassword(false);
								WorkLog.e("LoginService", "get password error");
								login_sdk
										.setLOGIN_RESON(LoginSDK.RESON_MOBILE_AUTH_ERROR);
								login_sdk.setLOGIN_STATUS(LoginSDK.LOGIN_ERROR);
								SendMessage();
	...
	public void onCreate() {
		super.onCreate();
		WorkLog.e("LoginService", "LoginService---OnCreate");
	...
	public void onDestroy() {
		super.onDestroy();
		WorkLog.e("LoginService", "LoginService---onDestroy");
	...
	private BroadcastReceiver mLoginStatusChangedReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			case LoginApi.STATUS_DISCONNECTED:
				...
				SendMessage();
				WorkLog.e("LoginService", "登录失败");
				WorkLog.e("reason", reasonCode);
				break;
			case LoginApi.STATUS_CONNECTED:
				WorkLog.e("LoginService", "登录成功");
