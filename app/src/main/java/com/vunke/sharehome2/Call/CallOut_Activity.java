package com.vunke.sharehome2.Call;
...
import com.vunke.sharehome2.model.Rx_ReLogin;
import com.vunke.sharehome2.receiver.CallStatusChangedReceive;
import com.vunke.sharehome2.rx.RxBus;
...
public class CallOut_Activity extends BaseActivity {
	 ...
	 * 电话号码， 通话状态 ，时间
	 */
	private TextView call_phoneName, callout_status, callout_time,
			call_phoneNumber;

	/**
	 * 切换视频开关 ,静音 ， 挂断 拨号键盘
	 */
	private Button callout_switch_video, callout_mute, callout_cancel,
			call_handsfree;
	...
	private void initCall() {
		...
		case CallSession.ERRCODE_SERVER_DISCONNECTED:
			WorkLog.e("CallOut_Activity",
					"CallSession.ERRCODE_SERVER_DISCONNECTED");
	...
	private void initViews() {
		...
		SetOnClickListener(callout_switch_video, callout_mute, callout_cancel,
				call_handsfree);
	}
	...
	private void registerReceivers() {
		callStatusChangedReceive2 = new CallStatusChangedReceive(mcontext,
				callSession, CallNumber, call_phoneName);
		LocalBroadcastManager.getInstance(getApplicationContext())
				.registerReceiver(callStatusChangedReceive2,
	...
	private void unregisterReceivers() {
		LocalBroadcastManager.getInstance(getApplicationContext())
				.unregisterReceiver(callStatusChangedReceive2);
	}

	@Override
	public void OnClick(View v) {
		if (v.getId() == R.id.callout_switch_video) {

		} else if (v.getId() == R.id.callout_mute) {
			isMute = !isMute;
			if (isMute) {
				callSession.mute();
		...
		} else if (v.getId() == R.id.callout_cancel) {
			if (callSession.getErrCode() != CallSession.ERRCODE_OK) {
				finish();
				return;
			}
		...
		} else if (v.getId() == R.id.call_handsfree) {
	...
	public void MuteDrawableTop(boolean b) {
		if (b) {
			drawable = getResources().getDrawable(R.mipmap.call_mute);
			drawable.setBounds(0, 0, drawable.getMinimumWidth(),
					drawable.getMinimumHeight());
			callout_mute.setCompoundDrawables(null, drawable, null, null);
		} else {
			drawable = getResources().getDrawable(R.mipmap.call_mute2);
			drawable.setBounds(0, 0, drawable.getMinimumWidth(),
					drawable.getMinimumHeight());
			callout_mute.setCompoundDrawables(null, drawable, null, null);
		}
